...
class LSTM:
    ...
    def _initialize_weights(self, n_features: int) -> None:
        ...
        for i in range(len(self.hidden_layers)):
            ...
            for gate in ['f', 'i', 'o', 'g']:
                self.W[f'W_{gate}_{i}'] = np.random.randn(prev_layer_size, h_size) * scale_x
                self.U[f'U_{gate}_{i}'] = np.random.randn(h_size, h_size) * scale_h
                self.b[f'b_{gate}_{i}'] = np.zeros((1, h_size))
